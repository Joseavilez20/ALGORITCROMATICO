...
##        general.append(k)
    ...
    A = A.tolist()
    B = B.tolist()
    muestra = muestra[:,0].tolist()
##    muestra = ['ND']*len(muestra)
   
    
    A.insert(0, 'Valor Real')
    B.insert(0, 'Pronosticado')
    
    meses = [i for i in range(1,len(A)+1)]
    meses.insert(0,'Mes')
    
    for i in zip(meses,A,B):
        lista.append(i)
    datos = tuple(lista)

    pronosticos = B[len(A):]
    for j in zip(muestra,pronosticos):
        lista2.append(j)
    dataPronosticoMuestra = tuple(lista2)
    ...
    tabla2 = Table(data = dataPronosticoMuestra,colWidths=[1*inch,3*inch,3*inch,3*inch, 3*inch],
                  style = [
                           ('GRID',(0,0),(-1,-1),0.5,colors.grey),
                           ('BOX',(0,0),(-1,-1),2,colors.black),
                           ('BACKGROUND', (0, 0), (-1, 0), colors.lavender),
    ...
    story.append(Paragraph('Coeficiente of determinación R^2: <font face="times" color="blue"> '+str(R2)+'</font>' ,styleH))
    story.append(Paragraph('Pronóstico de Datos Historicos' ,styleH))
   
    story.append(tabla)
    story.append(Spacer(0,15))
    
    story.append(Paragraph('Pronóstico Muestra de Datos' ,styleH))
